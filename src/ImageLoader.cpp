...
void StbImageLoader::LoadImage(const char* path) { data = stbi_load(path, &width, &height, &nrChannels, 0); }
void StbImageLoader::LoadImage(const char* path, int desiredChannels) { data = stbi_load(path, &width, &height, &nrChannels, desiredChannels); }
